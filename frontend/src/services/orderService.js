...
export const orderService = {
  ...
  checkInventory: async (cartItems) => {
    try {
      ...
      // Nếu là lỗi khác (network, server error, etc.)
      throw new Error(
        error.response?.data?.message || "Không thể kiểm tra tồn kho"
      );
    }
  },
  ...
  checkRushOrderEligibility: async (deliveryInfo, products) => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Không thể kiểm tra rush order"
      );
    }
  },
  ...
  getMockOrders: () => {
    return [
      ...
      },
      {
        ...
        totalAmount: 1150000,
        shippingAddress:
          "987 Dien Bien Phu, District Binh Thanh, Ho Chi Minh City",
        items: [
          { productName: "Spring Boot Guide", quantity: 1, price: 650000 },
          {
            productName: "Classical Music Collection",
            quantity: 1,
            price: 500000,
          },
        ],
