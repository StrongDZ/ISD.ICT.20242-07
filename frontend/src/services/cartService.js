...
export const cartService = {
  ...
  checkRushEligibility: async (deliveryInfo, cartItems) => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to check rush eligibility"
      );
    }
  },
  ...
  getOrderStatistics: () => {
    ...
    return {
      ...
      pendingOrders: orders.filter((o) => o.status === "PENDING_APPROVAL")
        .length,
  ...
  mockVNPayPayment: async (paymentData) => {
    ...
    if (success) {
      return {
        ...
        )}&vnp_OrderType=other&vnp_ReturnUrl=${encodeURIComponent(
          paymentData.returnUrl
        )}&vnp_TmnCode=DEMO&vnp_TxnRef=${
  ...
  validateOrderData: (orderData) => {
    ...
    if (
      ...
      orderData.deliveryAddress.trim().length < 10
    ) {
    ...
    if (orderData.isRushOrder) {
      ...
      if (
        ...
        !hanoiInnerCityDistricts.some((district) =>
          orderData.deliveryAddress.toLowerCase().includes(district)
        )
      ) {
